        ...
        {SOCIOS.map((s) => {
          ...
          return (
            <div key={s} className="g-card p-4">
              ...
              <p className="text-xs text-neutral-500">Puso en total</p>
              <p className="text-lg mb-2">{formatCurrency(cuentas[s].puso)}</p>
              <p className="text-xs text-neutral-500">Le toca cobrar</p>
              <p className="text-lg text-emerald-600">{formatCurrency(cuentas[s].cobra)}</p>
              {Math.abs(diff) > 1 && (
